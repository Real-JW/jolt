...
pub fn prove_mega_logup_circuit(
    ...
    for desc in circ.lut_types.values() {
        assert_eq!(
            desc.k, k,
            "Phase 4h requires uniform k across all LUT types"
        );
        assert_eq!(
            desc.m, m,
            "Phase 4h requires uniform m across all LUT types"
        );
    }
    ...
    debug_assert_eq!(
        1usize << m_table,
        mega_size,
        "mega_size must be a power of 2"
    );
    ...
    let comm_type_idx = HyperKZG::<Bn254>::commit(pk, &mle_type_idx).expect("commit TypeIdx");
    let comm_packed_in = HyperKZG::<Bn254>::commit(pk, &mle_packed_in).expect("commit PackedIn");
    let comm_packed_out = HyperKZG::<Bn254>::commit(pk, &mle_packed_out).expect("commit PackedOut");
    let comm_count = HyperKZG::<Bn254>::commit(pk, &mle_count).expect("commit Count");
    ...
    let open_one = |mle: &MultilinearPolynomial<Fr>,
     ...
     -> Option<HyperKZGProof<Bn254>> {
        if *comm == zero_comm {
            ...
            Some(HyperKZG::<Bn254>::open(pk, mle, pt, eval, t).expect("HyperKZG open failed"))
        }
    };
...
pub fn verify_mega_logup_circuit(
    ...
    for round in 0..m_query {
        ...
        if p.len() != 4 {
            eprintln!(
                "mega_logup verify: query sc round {round}: poly len {} ≠ 4",
                p.len()
            );
            return false;
        }
    ...
    if expected_q != prev_q {
        eprintln!("mega_logup verify: query sc final check FAILED: {expected_q:?} ≠ {prev_q:?}");
        return false;
    }

    for &v in proof.finals_q.iter() {
        transcript.append_scalar(&v);
    }
    ...
    for round in 0..m_table {
        ...
        if p.len() != 4 {
            eprintln!(
                "mega_logup verify: table sc round {round}: poly len {} ≠ 4",
                p.len()
            );
            return false;
        }
    ...
    let expected_t = eq_t_fin * (inv_t_f * (lambda - b_f) - Fr::one()) + xi * count_f * inv_t_f;
    if expected_t != prev_t {
        eprintln!("mega_logup verify: table sc final check FAILED: {expected_t:?} ≠ {prev_t:?}");
        return false;
    }

    for &v in proof.finals_t.iter() {
        transcript.append_scalar(&v);
    }
